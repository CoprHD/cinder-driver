...
class EMCCoprHDDriverCommon(object):
    ...
    def check_for_setup_error(self):
        ...
        if self.configuration.hostname is None:
            message = _("hostname is not set in cinder configuration")
            ...
            for try_id in range(1, EXPORT_RETRY_COUNT + 1):
                ...
                try:
                    ...
                    if (try_id >= EXPORT_RETRY_COUNT):
                        ...
                        LOG.exception(_LE("Export volume with LUN: %s failed.")
                                      % str(next_lun_id))
    ...
    def _get_coprhd_cgid(self, cgid):
        ...
        if len(rslt) > 0:
            ...
            raise CoprHD_utils.CoprHdError(
                ...
                "Consistency Group " + cgid + " not found")
    ...
    def update_volume_stats(self):
        ...
        try:
            ...
            LOG.error(_LE("Failed to update volume stats"))
            with excutils.save_and_reraise_exception():
