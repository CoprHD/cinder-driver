    ...
    def try_and_retry(*args, **kwargs):
        ...
        try:
            ...
            if (e.err_code == coprhd_utils.SOSError.HTTP_ERR and
                ...
                retry = True
                ViPRStorageConnection.AUTHENTICATED = False
            else:
                exception_message = "\nCoprHD Exception: %s\nStack Trace:\n%s" \
...
class ViPRStorageConnection(driver.StorageConnection):
    ...
    def __init__(self, share_backend_name, configuration=None):
        ...
        self._init_coprhd_cli_components()
    ...
    def allow_access(self, emc_share_driver, context, share, access,
        ...
        try:
            self.file_share_obj.export(
                ...
                "sys", 
                "rw",
                self.conn_configuration.emc_nas_login,
                [access['access_to']], 
                share_protocol,
                share_name=None,
    ...
    def check_for_setup_error(self, emc_share_driver):
        ...
        if self.conn_configuration.emc_nas_server is None:
            message = "emc_nas_server is not set in manila configuration"
            raise exception.ShareBackendException(data=message)

        if self.conn_configuration.emc_nas_server_port is None:
            message = "emc_nas_server_port is not set in manila configuration"
            raise exception.ShareBackendException(data=message)

        if self.conn_configuration.emc_nas_login is None:
            message = "emc_nas_login is not set in manila configuration"
            raise exception.ShareBackendException(data=message)

        if self.conn_configuration.emc_nas_password is None:
            message = "emc_nas_password is not set in manila configuration"
            raise exception.ShareBackendException(data=message)
                ...
                for vpair in vpairs:
                    if vpair:
                        (s, h) = coprhd_utils.service_json_request(
                            self.conn_configuration.emc_nas_server,
                            self.conn_configuration.emc_nas_server_port,
                            "GET",
    ...
    def _init_coprhd_cli_components(self):
    ...
    def _authenticate_user(self):
        ...
        if ViPRStorageConnection.AUTHENTICATED is False:
            obj = coprhd_auth.Authentication(
                self.conn_configuration.emc_nas_server,
                self.conn_configuration.emc_nas_server_port)

            username = self.conn_configuration.emc_nas_login
            password = self.conn_configuration.emc_nas_password
